...
class SpotterUI {
    ...
    const style = `
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* Tab Content */
        .tab-content { padding: 16px; display: none; overflow-y: auto; background: white; flex-grow: 1; min-height: 0; }
        .tab-content.active { display: block; }

        /* --- Controls Tab --- */
        .section-title { font-size: 11px; text-transform: uppercase; letter-spacing: 0.05em; color: #94a3b8; margin-bottom: 8px; font-weight: 700; }
        .main-toggle-btn { width: 100%; padding: 10px; border-radius: 8px; border: none; font-weight: 600; font-size: 14px; cursor: pointer; transition: background 0.2s; margin-bottom: 16px; display: flex; align-items: center; justify-content: center; gap: 8px; }
        ...
        .lists-container { display: flex; flex-direction: column; gap: 4px; }
        ...
        .nav-row.category-row { background: #e2e8f0; color: #334155; }
        .nav-row.group-row { background: #f1f5f9; color: #475569; }
        
        /* Prompt Nav (AI) */
        .nav-row.prompt-row { background: #e0f2fe; color: #1e40af; margin-bottom: 12px; }
        ...
        .forms-container { display: flex; flex-direction: column; gap: 10px; }
        ...
        .form-item.separator-item { background: transparent; border: none; padding: 5px 0; justify-content: center; }
        ...
        /* --- AI Tab Styles --- */
        .ai-container { display: flex; flex-direction: column; gap: 12px; height: 100%; }
        .ai-input, .ai-output { width: 100%; padding: 10px; border: 1px solid #cbd5e1; border-radius: 8px; font-family: inherit; font-size: 14px; resize: vertical; box-sizing: border-box; }
        .ai-input { min-height: 80px; }
        .ai-output { min-height: 120px; background: #f8fafc; color: #334155; }
        .ai-btn { background: #3b82f6; color: white; border: none; padding: 10px; border-radius: 8px; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: background 0.2s; }
        .ai-btn:hover { background: #2563eb; }
        .ai-btn:disabled { background: #94a3b8; cursor: not-allowed; }
        .ai-status { font-size: 12px; color: #ef4444; text-align: center; min-height: 16px; }
      </style>
    `;
  ...
  bindEvents() {
    ...
    // Prompt Navigation
    const prevPromptBtn = this.shadowRoot.getElementById("prev-prompt-btn");
    const nextPromptBtn = this.shadowRoot.getElementById("next-prompt-btn");

    prevPromptBtn.addEventListener("click", (e) => { e.stopPropagation(); this.changePrompt(-1); });
    nextPromptBtn.addEventListener("click", (e) => { e.stopPropagation(); this.changePrompt(1); });
    ...
    aiBtn.addEventListener("click", async (e) => {
        ...
        // Send to Background Script with prompt content
        chrome.runtime.sendMessage({ 
            action: "askAI", 
            text: text, 
            promptContent: promptContent // Passing prompt specific content
        }, (response) => {
            ...
            if (chrome.runtime.lastError) {
                aiStatus.textContent = "Erreur de communication avec l'extension.";
                return;
            }

            if (response.error) {
                aiStatus.textContent = response.error;
            } else {
  ...
  closeMenu() {
      this.isOpen = false;
      this.isLocked = false;
      this.shadowRoot.getElementById("spotter-menu").classList.remove("open");
      this.updateFabVisuals();
  }
  ...
  updateFabVisuals() {
      ...
      if (this.isLocked) {
          ...
          content.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>`;
      } else {
          fab.classList.remove("locked");
          content.innerHTML = `<img src="${iconUrl}" alt="Spotter" draggable="false">`;
      }
  }

  // ... (createHost and setupLongPressDrag remain unchanged) ...
  createHost(position) {
  ...
  refreshAIView() {
      ...
      if (this.aiPrompts.length === 0) {
          titleEl.textContent = "Aucun prompt configuré";
  ...
  refreshFormsView() {
      ...
      if (this.formCategories.length === 0) {
          catTitle.textContent = "Aucune catégorie";
          groupTitle.textContent = "-";
          container.innerHTML = '<div style="color:#94a3b8; text-align:center; padding:15px; font-size:13px;">Rien à afficher</div>';
          prevCat.disabled = nextCat.disabled = prevGroup.disabled = nextGroup.disabled = true;
          return;
      }
      ...
      if (!currentCat.groups || currentCat.groups.length === 0) {
          ...
          prevGroup.disabled = nextGroup.disabled = true;
          return;
      }
      ...
      if (validFields.length === 0) {
          container.innerHTML = '<div style="color:#94a3b8; text-align:center; padding:15px; font-size:13px;">Formulaire vide</div>';
          return;
      }
  ...
  // ... (renderLists and updateState methods remain unchanged)
  renderLists(lists) {
